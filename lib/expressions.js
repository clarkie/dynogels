...
// DynamoDB expression attribute names must contain only alphanumeric
// characters.  We map any non-alpha character to its four-hex-digit code
// representation -- including numbers, to avoid clashes.  ("foo-" and
// "foo002b" would otherwise map the same.)
internals.createNameKey = (attr) =>
    // eslint-disable-next-line prefer-template
    '#' + _.chain(attr)
        .map(i => (
            (i >= 'A' && i <= 'Z') || (i >= 'a' && i <= 'z') ?
            i :
            leftPad(i.charCodeAt(0).toString(16), 4, '0')
        ))
        .join('')
        .value();
...
exports.serializeUpdateExpression = (schema, item) => {
  ...
  const result = _.reduce(data, (result, value, key) => {
    const valueKey = `:${key}`;
    const nameKey = internals.createNameKey(key);
...
exports.buildFilterExpression = (key, operator, existingValueNames, val1, val2) => {
  ...
  const path = internals.createNameKey(key);
