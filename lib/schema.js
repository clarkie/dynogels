...
const Schema = module.exports = function (config) {
  ...
  const self = this;
  const { value: data, error } = internals.configSchema.validate(config, { context: context });
  if (error) {
    const msg = 'Invalid table schema, check your config ';
    throw new Error(msg + error.annotate());
  }
...
Schema.types.uuid = () => Joi.string().guid().default(() => nodeUUID.v4());

Schema.types.timeUUID = () => Joi.string().guid().default(() => nodeUUID.v1());
