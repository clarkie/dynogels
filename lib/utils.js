...
function getThroughputLimitDelay(consumedCapacityToRemoveToken, consumeThroughputLimit) {
  return Math.min(1000, Math.ceil((consumedCapacityToRemoveToken / consumeThroughputLimit) * 1000));
}

const removeMaxAvailableTokens = (self, consumedCapacity) =>
  self.rateLimiter.getTokensRemaining()
    .then(remainingTokens => {
      const tokensToRemove = Math.min(consumedCapacity, remainingTokens);
      return self.rateLimiter.tryRemoveTokens(tokensToRemove)
        .then(removed => {
          if (removed) {
...
utils.paginatedRequest = (self, runRequestFunc, callback) => {
  ...
  let consumedCapacityToRemoveToken = 0;
  ...
  const doFunc = callback => {
    ...
    const delayableRunFunc = () => {
      Promise.resolve()
        .then(() => {
          if (consumedCapacityToRemoveToken > 0) {
            return removeMaxAvailableTokens(self, consumedCapacityToRemoveToken);
          }
          return 0;
        })
        .then(tokensRemoved => {
          consumedCapacityToRemoveToken -= tokensRemoved;
          // check again
          if (consumedCapacityToRemoveToken > 0) {
            const delay = getThroughputLimitDelay(consumedCapacityToRemoveToken, self.rateLimiter.config.tokensPerSecond);
            ...
            Promise.resolve()
              .then(() => {
                ...
                if (consumedCapacity && self.rateLimiter) {
                  ...
                  return removeMaxAvailableTokens(self, consumedCapacity.CapacityUnits)
                    .then(tokensRemoved => (consumedCapacityToRemoveToken -= tokensRemoved));
                }
              })
