...
internals.keyCondition = (keyName, schema, query, customBinding) => {
  const f = operator => function () {
    ...
    return query.addKeyCondition(cond, customBinding);
  };
...
internals.queryFilter = (keyName, schema, query, customBinding) => {
  const f = operator => function () {
    const copy = [].slice.call(arguments);
    const existingValueKeys = _.keys(query.request.ExpressionAttributeValues);
    const args = [keyName, operator, existingValueKeys].concat(copy);
    ...
    return query.addFilterCondition(cond, customBinding);
  };
...
Query.prototype.addKeyCondition = function (condition, customBinding) {
...
Query.prototype.addFilterCondition = function (condition, customBinding) {
  internals.addExpressionAttributes(this.request, condition);
...
Query.prototype.where = function (keyName, customBinding) {
  return internals.keyCondition(keyName, this.table.schema, this, customBinding);
};

Query.prototype.filter = function (keyName, customBinding) {
  return internals.queryFilter(keyName, this.table.schema, this, customBinding);
};
