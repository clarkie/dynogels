...
module.exports = function(credentials) {
  ...
  exp.parse = str => _.reduce(internals.actionWords, (result, actionWord) => {
    result[actionWord] = internals.match(actionWord, str);
    return result;
  }, {});

  exp.serializeUpdateExpression = (schema, item) => {
  ...
  exp.stringify = expressions => _.reduce(expressions, (result, value, key) => {
    if (!_.isEmpty(value)) {
      if (_.isArray(value)) {
  ...
  exp.buildFilterExpression = (key, operator, existingValueNames, val1, val2) => {
