...
internals.keyCondition = (keyName, schema, query, customBinding, parenthese) => {
  const f = operator => function () {
    ...
    return query.addKeyCondition(cond, customBinding, parenthese);
  };
...
internals.queryFilter = (keyName, schema, query, customBinding, parenthese) => {
  const f = operator => function () {
    const copy = [].slice.call(arguments);
    const existingValueKeys = _.keys(query.request.ExpressionAttributeValues);
    const args = [keyName, operator, existingValueKeys].concat(copy);
    ...
    return query.addFilterCondition(cond, customBinding, parenthese);
  };
...
Query.prototype.addKeyCondition = function (condition, customBinding, parenthese) {
  internals.addExpressionAttributes(this.request, condition);

  if (_.isString(this.request.KeyConditionExpression) && customBinding) {
    if (parenthese === 'start') {
      this.request.KeyConditionExpression = `${this.request.KeyConditionExpression} ${customBinding} ((${condition.statement})`;
    }
  ...
  } else if (_.isString(this.request.KeyConditionExpression)) {
...
Query.prototype.addFilterCondition = function (condition, customBinding, parenthese) {
  internals.addExpressionAttributes(this.request, condition);

  if (_.isString(this.request.FilterExpression) && customBinding) {
    if (parenthese === 'start') {
      this.request.FilterExpression = `${this.request.FilterExpression} ${customBinding} ((${condition.statement})`;
    }
  ...
  } else if (_.isString(this.request.FilterExpression)) {
...
Query.prototype.where = function (keyName, customBinding, parenthese) {
  return internals.keyCondition(keyName, this.table.schema, this, customBinding, parenthese);
};

Query.prototype.filter = function (keyName, customBinding, parenthese) {
  return internals.queryFilter(keyName, this.table.schema, this, customBinding, parenthese);
};
