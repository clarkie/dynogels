...
internals.keyCondition = (keyName, schema, scan, customBinding) => {
  const f = operator => function () {
    ...
    return scan.addFilterCondition(cond, customBinding);
  };
...
Scan.prototype.addFilterCondition = function (condition, customBinding) {
  ...
  if (_.isString(this.request.FilterExpression) && customBinding) {
    this.request.FilterExpression = `${this.request.FilterExpression} OR (${condition.statement})`;
...
Scan.prototype.where = function (keyName, customBinding) {
