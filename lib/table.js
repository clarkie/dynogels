'use strict';
const { QueryCommand, ScanCommand, BatchGetItemCommand, PutItemCommand, UpdateItemCommand, DeleteItemCommand, CreateTableCommand, DescribeTableCommand, DeleteTableCommand, UpdateTableCommand } = require("@aws-sdk/client-dynamodb");
...
module.exports = function(credentials) {
  const Query = require('./query')(credentials);
  const Scan = require('./scan')(credentials);
  const ParallelScan = require('./parallelScan')(credentials);
  const expressions = require('./expressions')(credentials);

  const Table = function (name, schema, serializer, docClient, logger) {
  ...
  Table.prototype.sendRequest = async function (method, params, callback) {
    const self = this;
    const  driver = self.docClient;
    const startTime = Date.now();
    const marshallOptions = {'removeUndefinedValues': true};

    if (params.ExpressionAttributeValues) {
      params.ExpressionAttributeValues = marshall(params.ExpressionAttributeValues, marshallOptions);
    }
    if (params.Key) {
      params.Key = marshall(params.Key, marshallOptions);
    }

    self.log.info({ params: params }, 'dynogels %s request', method.toUpperCase());
    let fn;
    if (method === 'get') {
      fn = new GetCommand(params);
    } else if (method === 'create') {
      fn = new PutItemCommand(params);
    } else if (method === 'put') {
      fn = new PutCommand(params);
    } else if (method === 'update') {
      fn = new UpdateItemCommand(params);
    } else if (method === 'delete') {
      fn = new DeleteItemCommand(params);
    } else if (method === 'query') {
      fn = new QueryCommand(params);
    } else if (method === 'scan') {
      fn = new ScanCommand(params);
    } else if (method === 'batchGet') {
      fn = new BatchGetItemCommand(params);
    } else if (method === 'createTable') {
      fn = new CreateTableCommand(params);
    } else if (method === 'describeTable') {
      fn = new DescribeTableCommand(params);
    } else if (method === 'deleteTable') {
      fn = new DeleteTableCommand(params);
    } else if (method === 'updateTable') {
      fn = new UpdateTableCommand(params);
    }
    try {
      let data = await driver.send(fn);
      const elapsed = Date.now() - startTime;
      self.log.info({ data: data }, 'dynogels %s response - %sms', method.toUpperCase(), elapsed);
      if (data.Items) {
        data.Items = data.Items.map((i) => unmarshall(i));
      }
      return callback(null, data);
    } catch (error) {
      self.log.warn({ err: error }, 'dynogels %s error', method.toUpperCase());
