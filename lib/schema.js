...
internals.configSchema = Joi.object().keys({
  ...
  rangeKey: Joi.string(),
  tableName: Joi.alternatives().try(Joi.string(), Joi.function()),
...
internals.findDynamoTypeMetadata = (data) => {
  const meta = _.find(data.metas, data => _.isString(data.dynamoType));
  if (meta) {
...
const Schema = module.exports = function (config) {
  ...
  const self = this;
  const result = internals.configSchema.validate(config, { context });
  const { error: err, value: data } = result;
  if (err) {
...
Schema.types.uuid = () => Joi.string().guid().default(() => v4());

Schema.types.timeUUID = () => Joi.string().guid().default(() => v1());
...
Schema.prototype.validate = function (params, options) {
  ...
  return this._modelSchema.validate(params);
};
