...
Table.prototype.update = function (item, options, callback) {
  ...
  internals.callBeforeHooks(self, 'update', start, (err, data) => {
    ...
    let params = {
      ...
      ExpressionAttributeNames: { '#hashKey': self.schema.hashKey },
    ...
    if (exp.ExpressionAttributeValues) {
      params.ExpressionAttributeValues = _.merge({}, params.ExpressionAttributeValues, exp.ExpressionAttributeValues);
      delete options.ExpressionAttributeValues;
    }

    if (exp.ExpressionAttributeNames) {
      params.ExpressionAttributeNames = _.merge({}, params.ExpressionAttributeNames, exp.ExpressionAttributeNames);
      delete options.ExpressionAttributeNames;
    }

    if (options.ConditionExpression && params.ConditionExpression) {
      params.ConditionExpression =
        `${params.ConditionExpression} AND ${options.ConditionExpression}`;
      delete options.ConditionExpression;
    }
