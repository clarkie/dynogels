...
internals.createItem = (table, item, options, callback) => {
  ...
  internals.callBeforeHooks(self, 'create', start, (err, data) => {
    ...
    let omitnullkeys = _.keys(_.pickBy(self.schema._modelDatatypes, (dt) => {
      return dt !== 'L';
    }));
    let omitted = utils.omitNulls(_.pick(data, omitnullkeys));
    const attrs = _.assign(data, omitted);
    ...
    self.sendRequest('put', params, (err, response) => {
      if (err) {
        return callback(err);
      }
      ...
      return callback(null, item, response);
    });
  });
};
